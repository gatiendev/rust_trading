...
- **Live data persistence** – Newly arriving closed candles are merged into an in‑memory rolling window (up to 50,000 candles) and persisted in two ways:
  - **Appended to a CSV file** (e.g., `m5_latest_50000_raw.csv`) for a permanent, ever‑growing log of all candles.
...
On startup, the bot loads (or fetches) the latest 50,000 candles from Binance and stores them in a rolling window. For every new closed candle received via WebSocket:
    ...
    The candle is appended to a CSV file (e.g., data/m5_latest_50000_raw.csv).

    The rolling window is updated (oldest candle dropped if needed) and the entire window is saved to a Parquet file (e.g., data/m5_latest_50000.parquet), overwriting the previous version.
...
Manages the WebSocket connection, parses incoming messages, and maintains the rolling window of candles. On each new closed candle, it updates the window, appends the candle to the CSV log, and overwrites the Parquet cache with the updated window.
main.rs
...
🧪 Development Roadmap
    ...
    WebSocket streaming from Binance (trades, 5m, 15m) with readable timestamps

    Docker containerization (production & development)

    Historical data fetching and caching (Parquet)

    Modular code structure

    Live data persistence (CSV append + Parquet rolling window)
...
This update adds the new persistence features to the "New Features" section, updates the architecture diagram to show the CSV log, and clarifies the behavior in "Usage" and module descriptions. The roadmap now includes the completed "Live data persistence" item.
