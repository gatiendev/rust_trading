...
fn ewma_opts_from_span(span: usize) -> EWMOptions {
    ...
    EWMOptions {
        alpha: Some(alpha), // <-- fixed: wrap in Some()
        adjust: true,
        bias: false,
        min_periods: span,
        ignore_nulls: false,
...
fn compute_resampled_ema(
    ...
    let every = Duration::parse(interval)?; // <-- propagate error
    let period = Duration::parse(interval)?; // <-- propagate error
    let offset = Duration::parse("0ns")?; // <-- propagate error
