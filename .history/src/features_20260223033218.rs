...
pub fn compute_features(klines: &[Kline]) -> Result<DataFrame> {
    ...
    // EWMOptions in Polars 0.53 takes optional fields; we only set span.
    let ema50_opts = EWMOptions {
        span: Some(50.0),
        ..Default::default()
    };
    let ema200_opts = EWMOptions {
        span: Some(200.0),
        ..Default::default()
    };
...
fn compute_resampled_ema(
    ...
    let ema_opts = EWMOptions {
        span: Some(span as f64),
        ..Default::default()
    };
    let ema = resampled
        .lazy()
