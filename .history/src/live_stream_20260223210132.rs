                    ...
                    "m5" | "m15" => {
                        if let Some(kline) = data["k"].as_object() {
                            ...
                            if let (
                                ...
                                // Append the latest feature row to the ever‑growing log
                                utils::measure_time("append feature row (feature csv)", || {
                                    data_storage::append_features_row_to_csv(
                                        &features_df,
                                        feature_csv,
                                    )
                                })?;
                                ...
                                // --- Append the latest feature row to the ever‑growing log ---
                                utils::measure_time("append feature row (streaming csv)", || {
                                    data_storage::append_features_row_to_csv(
                                        &features_df,
                                        streaming_csv_file,
                                    )
                                ...
                                // --- Persist raw data ---
                                // Append to CSV
                                utils::measure_time("append raw kline", || {
                                ...
                                if let Err(e) =
                                    data_storage::save_klines_to_parquet(&raw_slice, raw_cache_file)
                                {
