...
/// Run the live stream. `historical` is the initial data window.
...
pub async fn run(
    ...
    let features_df = features::compute_features(&feature_window, &[10, 20, 50])?;
    ...
    println!("Loaded {} historical klines for context.", historical.len());
                    ...
                    "m5" | "m15" => {
                        if let Some(kline) = data["k"].as_object() {
                            ...
                            if let (
                                ...
                                historical.push(new_kline.clone());
                                if historical.len() > HISTORICAL_COUNT {
                                    // Remove oldest (front)
                                    historical.remove(0);
                                }
                                ...
                                if let Err(e) =
                                    data_storage::save_klines_to_parquet(&historical, cache_file)
                                {
