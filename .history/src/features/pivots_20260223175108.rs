...
pub fn add_pivot_features(mut df: DataFrame) -> Result<DataFrame> {
    ...
    let cols = vec![
        Series::new("pivot_high_left".into(), left_high).into(),
        Series::new("pivot_high_right".into(), right_high).into(),
        Series::new("pivot_low_left".into(), left_low).into(),
        Series::new("pivot_low_right".into(), right_low).into(),
    ];
    df.with_columns(cols)?; // modifies df in place, returns &mut Self – we ignore it
