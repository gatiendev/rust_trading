...
pub fn add_pivot_features(mut df: DataFrame) -> Result<DataFrame> {
    ...
    df.with_column(Series::new("pivot_high_left", left_high).into())?;
    df.with_column(Series::new("pivot_high_right", right_high).into())?;
    df.with_column(Series::new("pivot_low_left", left_low).into())?;
    df.with_column(Series::new("pivot_low_right", right_low).into())?;
