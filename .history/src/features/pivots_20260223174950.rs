...
pub fn add_pivot_features(mut df: DataFrame) -> Result<DataFrame> {
    ...
    let cols = vec![
        Column::new("pivot_high_left".into(), left_high),
        Column::new("pivot_high_right".into(), right_high),
        Column::new("pivot_low_left".into(), left_low),
        Column::new("pivot_low_right".into(), right_low),
    ];
    df = df.with_column(cols)?;
