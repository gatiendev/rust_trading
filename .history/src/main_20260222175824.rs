                    ...
                    "m5" | "m15" => {
                        if let Some(kline) = data["k"].as_object() {
                            ...
                            if let (
                                ...
                                kline["c"].as_str(),
                                kline["v"].as_str(),
                                kline["t"].as_u64(), // open time
                                kline["T"].as_u64(), // close time
                            ) {
                                let open_time_str = format_time(open_time);
                                let close_time_str = format_time(close_time);
                                println!(
                                    "Kline | OpenTime: {} | CloseTime: {} | Open: {} | High: {} | Low: {} | Close: {} | Volume: {}",
                                    open_time_str, close_time_str, open, high, low, close, volume
                                );
                            }
                        }
                    }
