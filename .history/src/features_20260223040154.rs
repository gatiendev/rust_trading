...
pub fn compute_features(klines: &[Kline]) -> Result<DataFrame> {
    ...
    // Left join each resampled EMA on datetime (exact match), then forward‑fill.
    let mut result = df.clone();
    let ema_dfs = [h1_ema50, h1_ema200, h4_ema50, h4_ema200];
    let ema_col_names = ["ema50_h1", "ema200_h1", "ema50_h4", "ema200_h4"];

    for (ema_df, col_name) in ema_dfs.iter().zip(ema_col_names.iter()) {
        result = result
            .lazy()
            .join(
                ema_df.clone().lazy(),
    ...
    // Forward‑fill the new EMA columns (propagate last known value)
    let mut lazy = result.lazy();
    for col_name in &ema_col_names {
        lazy = lazy.with_column(col(col_name).fill_null(FillNullStrategy::Forward(None)));
    }
    result = lazy.collect()?;
