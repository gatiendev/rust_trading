...
async fn load_or_fetch_historical() -> Result<Vec<kline::Kline>> {
    ...
    if should_fetch {
        ...
        let df_result = data_storage::klines_to_dataframe(&klines);
        let mut df = match df_result {
            Ok(df) => df,
            Err(e) => {
                eprintln!("ERROR: Failed to convert klines to DataFrame: {}", e);
                // Return an empty vector to let the program continue (or panic)
                return Ok(klines); // but this may hide the real issue
            }
        };
        // let mut df = data_storage::klines_to_dataframe(&klines)?;

        // if let Err(e) = data_storage::save_dataframe_csv(&mut df, "output.csv") {
