...
pub fn compute_features(klines: &[Kline]) -> Result<DataFrame> {
    ...
    df = df
        .lazy()
        .with_column(col("open_time").cast(DataType::Datetime(TimeUnit::Milliseconds, None)))
        .sort(vec!["open_time"], Default::default())
        .collect()?;
    ...
    for col_name in &ema_cols {
        ...
        let s = result_df.column(col_name)?.clone();
        // Fill forward
        let filled = s.fill_null(FillNullStrategy::Forward(None))?;
        // Replace the column
        result_df.replace(col_name, filled)?;
    }
...
fn compute_resampled_ema(
    ...
    let resampled = df
        .clone()
        .lazy()
        .group_by_dynamic(col("open_time"), [], options)
    ...
    let ema = resampled
        ...
        .select([col("open_time"), col(col_name)])
        .collect()?;
