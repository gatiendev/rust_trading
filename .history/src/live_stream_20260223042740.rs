...
/// - `feature_parquet` – Parquet file for feature window (overwritten)
/// - `feature_csv` – CSV file for feature window (overwritten)
pub async fn run(
    stream_type: &str,
    ...
    // Build initial feature window (last FEATURE_WINDOW_SIZE from raw_window)
    ...
    // Initial feature computation and save
    let mut features_df = features::compute_features(&feature_window)?;
    println!(
                    ...
                    "m5" | "m15" => {
                        if let Some(kline) = data["k"].as_object() {
                            ...
                            if let (
                                ...
                                let mut features_df = features::compute_features(&feature_window)?;
                                // Save feature DataFrame (overwrite)
