...
pub fn save_dataframe_csv_to_path(df: &mut DataFrame, path: &str) -> Result<()> {
    // Clone the DataFrame to avoid modifying the original
    let mut df_clone = df.clone();

    // Convert open_time and close_time to human‑readable strings if they exist
    for col_name in &["open_time", "close_time"] {
        if let Ok(series) = df_clone.column(*col_name) {
            if let Ok(ca) = series.i64() {
                let str_vals: Vec<String> = ca
                    .into_iter()
                    .map(|opt| opt.map_or(String::new(), timestamp_to_string))
                    .collect();
                let new_series = Series::new(PlSmallStr::from(*col_name), str_vals); // fix: use PlSmallStr
                df_clone.replace(*col_name, new_series.into())?; // fix: convert Series to Column via .into()
            }
        }
    }
