...
async fn main() -> Result<()> {
    ...
    match args.get(1).map(String::as_str) {
        Some("fetch-historical") => {
            ...
            let mut df = data_storage::klines_to_dataframe(&klines)?; // ← make df mutable
            data_storage::save_dataframe(&mut df, output)?;
            println!("Saved to {}", output);
        }
        _ => {
            // Default / live mode: load latest 50k M15 candles, then start live stream
            let historical = load_or_fetch_historical().await?;
            println!("Proceeding with {} historical klines.", historical.len());
...
async fn load_or_fetch_historical() -> Result<Vec<kline::Kline>> {
    ...
    if should_fetch {
        ...
        data_storage::save_dataframe(&mut df, CACHE_FILE)?;
        Ok(klines)
    } else {
